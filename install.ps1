# Installs the firefly CLI on Windows.
#
...
Write-Host ''
Write-Host 'firefly - Adobe Firefly from the command line' -ForegroundColor White
Write-Host ''
...
firefly setup
if ($LASTEXITCODE -ne 0) { Write-Host '    Skipped. Run "firefly setup" later.' -ForegroundColor Yellow }

Write-Host ''
Write-Ok 'Installed.'
Write-Host ''
Write-Host '  Open a NEW terminal, then run:' -ForegroundColor White
Write-Host '    firefly login'
Write-Host '    firefly image "a red fox in snow"'
Write-Host ''
