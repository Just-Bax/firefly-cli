...
def cmd_setup(ctx: Context) -> int:
    if browser_is_installed():
        emit(
            ctx, {"browser": "installed", "ready": True}, text="[green]Ready.[/green] firefly login"
        )
        return 0
    ...
    if ctx.args.check:
        emit(
            ...
            text="Browser not installed. Run: firefly setup",
        )
        return 1
    ...
    if not install_chromium():
        emit(
            ...
            text="[red]Download failed.[/red] Check your connection and run 'firefly setup' again.",
        )
        return 1

    emit(ctx, {"browser": "installed", "ready": True}, text="[green]Ready.[/green] firefly login")
    return 0
